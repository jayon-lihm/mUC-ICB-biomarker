...
onco_clinical_input <- data.frame(TMB_df[, c("patient_name", "n_missense", "mono_best_response_clean",
                                                   "combo_best_response_clean")])
rownames(onco_clinical_input) <- onco_clinical_input$patient_name
onco_clinical_input <- onco_clinical_input[ colnames(onco_input),]
onco_clinical_input$mono_best_response_clean <- factor(onco_clinical_input$mono_best_response_clean, levels=c("CR", "PR", "SD", "PD", "NE"))
onco_clinical_input$combo_best_response_clean <- factor(onco_clinical_input$combo_best_response_clean, levels=c("CR", "PR", "SD", "PD", "NE"))

## reorder based on clinical response
reordered_onco_clinical_input <- onco_clinical_input[order(onco_clinical_input$mono_best_response_clean), ]

## onco print graphic setting
source("./bioinf_scripts/src/oncoprint_graphic_setting.R")
source("./bioinf_scripts/src/oncoprint_functions.R")
...
reordered_input <- onco_input[, reordered_onco_clinical_input$sample_name]


pdf("./wes_data/summary_data/Supplemental_Figure_S6.pdf", width=13, height = 8)
oncoPrint(reordered_input,
          alter_fun = alter_fun,
          column_order = reordered_onco_clinical_input$sample_name,
          col = col,
          top_annotation = HeatmapAnnotation(cbar = anno_oncoprint_barplot(),
                                             TMB = anno_barplot(reordered_onco_clinical_input$n_missense),
                                             Mono = reordered_onco_clinical_input$mono_best_response_clean,
                                             Combo = reordered_onco_clinical_input$combo_best_response_clean,
                                             col = list( TMB = "grey",
                                                         Mono =  col_response,
                                                         Combo = col_response)
                                             ),
